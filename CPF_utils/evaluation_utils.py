...
def run_multiplication_acc_evaluation(
    ...
    for i in tqdm(range(0, len(samples), batch_size), desc=f"{'Extracting HS' if extract_hs else 'Evaluating'} {dataset_name} ({mode_str})"):
        ...
        for sample in batch_samples:
            if use_cot_prompt:
                user_prompt = (
                    f"You are a precise calculator. You must compute {digits_desc} multiplication "
                    "strictly following the standard long multiplication algorithm, step by step. "
                    "Do not skip steps, do not use shortcuts, and do not directly give the answer. "
                    "Structure your response exactly as follows, using numbered steps 1 to 5:\n"
                    ...
                    " 1716\n\n"
                    "5. FINAL ANSWER: 1716\n\n"
                    f"Now compute the following multiplication in exactly the same format (steps 1-5):\n"
                    f"{sample['prompt']}"
                )
            else:
                user_prompt = (
